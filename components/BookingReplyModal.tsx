...
interface BookingReplyModalProps {
  isOpen: boolean;
  onClose: () => void;
  bookingId: string;
 ...
 *   originalNotes: Original notes from customer
 *   onSuccess: Callback when reply is sent successfully
 */
export default function BookingReplyModal({
  isOpen,
  onClose,
  bookingId,
  customerEmail,
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch("/api/bookings/send-reply", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        if (data.migrationRequired) {
          errorMessage = `${errorMessage}\n\nBitte führen Sie die Migration aus: ${data.migrationFile}`;
        } else if (data.rlsIssue) {
          errorMessage = `${errorMessage}\n\nRLS-Problem: ${data.details || "Unbekannt"}`;
          if (data.code) {
            errorMessage += `\nFehlercode: ${data.code}`;
          }
          if (data.isAdmin === false) {
            errorMessage += `\n\nHinweis: Der Benutzer wurde nicht als Admin erkannt.`;
          }
        ...
        <div className="p-6">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="space-y-1 text-sm text-gray-600">
              ...
              <p><span className="font-medium">Kurs:</span> {courseTitle}</p>
              <p><span className="font-medium">Datum:</span> {bookingDate}</p>
              <p><span className="font-medium">Zeit:</span> {bookingTime}</p>
            </div>
          </div>
          ...
          {originalNotes && (
            <div className="mb-6 p-4 bg-amber-50 border-l-4 border-amber-500 rounded">
              <h3 className="font-semibold text-gray-900 mb-2">Originale Notiz des Kunden:</h3>
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{originalNotes}</p>
            </div>
          )}
